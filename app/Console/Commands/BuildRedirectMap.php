...
class BuildRedirectMap extends Command
{
    ...
    public function handle(): int
    {
        ...
        uasort($map, function ($a, $b) {
            return [$b['hits'], $a['source']] <=> [$a['hits'], $b['source']];
        });
    ...
    protected function loadGsc(string $file): array
    {
        ...
        foreach ($rows as $r) {
            $url = $r[0] ?? '';
            if (! preg_match('~https?://[^/]+(?P<path>/[^?\s#]*)~i', $url, $m)) {
                continue;
            }
            $paths[] = $m['path'];
        }
    ...
    protected function patternRedirect(string $p): array
    {
        ...
        if (preg_match('~^/(category|tag|author)/~', $p)) {
            return ['/blog', 301, 1.0, 'wp archive'];
        }
        if (preg_match('~^/blog/page/\d+$~', $p)) {
            return ['/blog', 301, 1.0, 'blog pagination'];
        }
        if (preg_match('~^/(feed|rss|atom)$~', $p) || preg_match('~^/blog/(feed|rss|atom)$~', $p)) {
            return ['/rss.xml', 301, 1.0, 'feed'];
        }
        if (preg_match('~^/(services|service|what-i-do|portfolio|work|projects)$~', $p)) {
            return ['/software', 301, 1.0, 'section rename'];
        }
        if (preg_match('~^/(contact-us|contact-me)$~', $p)) {
            return ['/contact', 301, 1.0, 'section rename'];
        }
        if ($p === '/about-me') {
            return ['/about', 301, 1.0, 'section rename'];
        }
        if (preg_match('~^/(wp-admin|wp-login\.php|wp-json|xmlrpc\.php)(/|$)~', $p)) {
            return ['', 410, 1.0, 'wp endpoint'];
        }
