...
<template>
    <AppLayout>
        ...
        <section class="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <button v-for="(p,i) in gallery.photos" :key="p.src" @click="show(i)"
                :aria-label="`View photo ${i + 1} of ${gallery.photos.length}${p.alt ? ': ' + p.alt : ''}`"
                class="group relative rounded-xl overflow-hidden border border-slate-200 bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500">
        ...
        <section v-if="related?.length" class="mt-12 sm:mt-16">
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <a v-for="r in related" :key="r.slug" :href="`/photography/${r.slug}`" class="block rounded-2xl border border-slate-200 bg-white overflow-hidden hover:shadow-lg">
                    <img v-if="r.cover" :src="r.cover" :alt="`${r.title} cover`" class="w-full aspect-[4/3] object-cover" loading="lazy" width="4" height="3" />
                    <div class="p-4">
